...
右侧应用的阿里云实例 ID 为 `0a3747d86559413eb88a83751070d208`。

### 0.2.4-creator.21 单节点升级兼容性

本版本包含当前本地创作中心代码。官方迁移 235 将 `groups.models_list_config`
改名为 `model_allowlist`，直接执行会让仍运行 0.2.1 的另一台应用查询失败。
新增的 `235_creator_model_allowlist_compat.sql` 按文件名排序先于官方 235 执行：
保留两列并通过触发器同步新旧应用的写入，空配置清除也双向同步。
已有两列非空且冲突时停止迁移，避免覆盖配置。保留兼容列和触发器直到另行安排清理，
应用回滚时不删除它们。原官方迁移及校验和保持不变。

该兼容性通过 `deploy/tests/group-model-allowlist-rolling-test.sql` 验证，覆盖旧结构、
已改名结构、官方迁移顺序、重复执行、双向增改清除和冲突拒绝。
本次更新仍必须完成中央数据库备份；迁移 238 会清理全部限额均为空的配额行。
仅升级右侧时，左侧继续运行现有镜像，不能以“两台版本不同”为由自动更新左侧。
