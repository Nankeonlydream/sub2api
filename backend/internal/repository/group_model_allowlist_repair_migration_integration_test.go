...
// These fixtures reproduce upstream-only historical schemas. The surrounding
// test transaction restores the Creator compatibility columns and trigger.
func withoutCreatorModelAllowlistCompatibility(ctx context.Context, t *testing.T, tx *sql.Tx) {
	t.Helper()
	_, err := tx.ExecContext(ctx, `
DROP TRIGGER IF EXISTS creator_sync_group_model_allowlist ON groups;
ALTER TABLE groups DROP COLUMN IF EXISTS models_list_config;
`)
	require.NoError(t, err)
}
