...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	// A status/content read belongs to an already accepted async task. Keep it
	// readable if that generation consumed the key's remaining balance; the
	// completion path below still records any deferred video charge exactly once.
